...
export default function butterfly_chart(container){
    ...
    var chart_title = outer_wrap.append("p").text("Chart");
    ...
    var pts; //points
    function draw(){
        var pts_up = g_points.selectAll("circle").data(chart_data, function(d){return d.fp});
        pts_up.exit().remove()
        pts = pts_up.enter().append("circle").merge(pts_up)
                    .interrupt()
                    .attr("cx",function(d){return scales.x(d.spw)})
                    .attr("cy",function(d){return scales.y(d.spb)})
                    .attr("r","1")
                    .attr("fill","blue")
